...
public class TemplatePDF {
    ...
    public void addImage(Bitmap bitmap){
        ...
        float imageStartX = width - document.rightMargin() - 485f;
        float imageStartY = height - document.topMargin() - 790f;
        ByteArrayOutputStream stream = new ByteArrayOutputStream();

        bitmap.compress(Bitmap.CompressFormat.JPEG, 100, stream);
        Image myImg;
        //put bitmap in template
        try{
            myImg = Image.getInstance(stream.toByteArray());
            myImg.setAlignment(Image.TEXTWRAP);
            myImg.scaleAbsolute(420f, 300f);
            myImg.setAbsolutePosition(imageStartX, imageStartY);
